...
export async function assertMinVersion(): Promise<void> {
  try {
    ...
    if (
      ...
      console.error(`
It looks like your version of Pixel Code (${MACRO.VERSION}) needs an update.
...
export async function installGlobalPackage(): Promise<InstallStatus> {
  ...
  try {
    ...
    if (installResult.code !== 0) {
      logError(
        `Failed to install new version of pixel: ${installResult.stdout} ${installResult.stderr}`,
      )
